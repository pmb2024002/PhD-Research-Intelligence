...
OUTPUT_FILE = "data/raw/pubmed_articles_batch_003.csv"
...
def main():
    ...
    # ---------------------------------------------------------
    # Exclude papers already present in Batch 002
    # ---------------------------------------------------------
    ...
    if batch_002_file.exists():

        batch_002 = pd.read_csv(
            batch_002_file,
            dtype={"pmid": str}
        )
        ...
        pmids = [
            str(pmid)
            for pmid in pmids
            if str(pmid).strip() not in existing_pmids
        ]

        print(
            f"PubMed candidates retrieved: {original_count}"
        )

        print(
            f"Batch-002 overlaps removed: "
            f"{original_count - len(pmids)}"
        )

        print(
            f"New Batch-003 candidates: {len(pmids)}"
        )

    else:

        print(
            "WARNING: Batch-002 file not found. "
            "No overlap filtering performed."
        )

    print(
        f"Proceeding with {len(pmids)} papers."
    )
    print(
